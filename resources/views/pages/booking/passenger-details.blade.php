    ...
    <form action="{{ route('booking.savePassengerDetails', $flight->flight_number) }}" class="flex gap-[30px] mt-[30px]"
        ...
        <div id="Left-Content" class="flex flex-col gap-[30px] w-[470px] shrink-0">
            <div id="Flight-Info" class="flex flex-col w-[470px] shrink-0 h-fit rounded-[20px] bg-white p-5 gap-5">
                ...
                <div class="flex justify-between">
                    <div>
                        <p class="text-sm text-garuda-grey">{{ $flight->segments->first()->time->format('d M Y') }}</p>
                ...
                <div class="accordion-content p-5 pt-0 flex flex-col gap-5">
                    <div class="flex justify-between">
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-lg leading-[27px] mt-[2px]">
                            {{ implode(', ', $flight->seats->whereIn('id', $transaction['selected_seats'])->pluck('name')->toArray()) }}</p>
                        </div>
                    </div>
                    <div class="flex justify-between">
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-semibold text-lg leading-[27px] mt-[2px]">
                        {{ 'Rp. ' . number_format($tier->price * count($transaction['selected_seats']), 0, ',', '.') }}</p>
                        </div>
                    </div>
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="font-semibold text-lg leading-[27px] mt-[2px]">
                        {{ 'Rp. ' . number_format($tier->price * count($transaction['selected_seats']) * 0.11, 0, ',', '.') }}</p>
                        </div>
                        <div>
                            <p class="text-sm text-garuda-grey">Grand Total</p>
                            <p class="font-bold text-2xl leading-9 text-garuda-blue mt-[2px]">
                                {{ 'Rp. ' . number_format($tier->price * count($transaction['selected_seats']) * 1.11, 0, ',', '.') }}</p>
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="accordion-content p-5 pt-0 flex flex-col gap-5">
            ...
            <label class="flex flex-col gap-[10px]">
                ...
                <div class="flex items-center rounded-full border border-garuda-black py-3 px-5 gap-[10px] focus-within:border-[#0068FF] transition-all duration-300 @error('name') border-red-500 @enderror">
                    ...
                    <!-- Value & readonly dihapus agar bisa diketik manual -->
            ...
            @foreach ($transaction['selected_seats'] as $seatId)
            ...
            <label class="flex flex-col gap-[10px]">
                ...
                @error('passengers.' . $loop->index + 1 . '.name') border-red-500 @enderror">
                    <img src="{{ asset('assets/images/icons/profile-black.svg') }}" class="w-5 flex shrink-0" alt="icon">
                    <input type="text" name="passengers[{{ $loop->index }}][name]"
                        class="appearance-none outline-none w-full font-semibold placeholder:font-normal"
                        placeholder="Write your complete name">
                </div>
                @error('passengers.' . $loop->index + 1 . '.name')
                <p class="text-sm text-red-500">{{ $message }}</p>
                @enderror
            </label>
            ...
            <div class="flex flex-col gap-[10px]">
                ...
                {{-- Input hidden penampung format YYYY-MM-DD --}}
               <input type="hidden" name="passengers[{{ $loop->index }}][date_of_birth]" id="date_of_birth-{{ $loop->index }}" data-index="{{ $loop->index }}">
                ...
                <div class="flex items-center gap-[10px]">
                    ...
                    <label class="relative flex items-center w-full rounded-full overflow-hidden border border-garuda-black gap-[10px] focus-within:border-[#0068FF] transition-all duration-300">
                        ...
                        <select id="day-select-{{ $loop->index }}" name=""
                            class="date-select day-select appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal cursor-pointer"
                            data-index="{{ $loop->index }}"
                            onchange="updateDateOfBirth({{ $loop->index }})">
                            <option hidden value="">DD</option>
                            
                            <!-- LOOP ANGKA TANGGAL DI SINI -->
                            @for ($i = 1; $i <= 31; $i++)
                    ...
                    <label class="relative flex items-center w-full rounded-full overflow-hidden border border-garuda-black gap-[10px] focus-within:border-[#0068FF] transition-all duration-300">
                        ...
                        <select id="month-select-{{ $loop->index }}" name=""
                            class="date-select month-select appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal cursor-pointer"
                            data-index="{{ $loop->index }}"
                            onchange="updateDateOfBirth({{ $loop->index }})">
                            <option hidden value="">MM</option>
                            
                            <!-- LOOP ANGKA BULAN DI SINI -->
                            @for ($i = 1; $i <= 12; $i++)
                                <option value="{{ sprintf('%02d', $i) }}">{{ sprintf('%02d', $i) }}</option>
                            @endfor
                        </select>
                    ...
                    <label class="relative flex items-center w-full rounded-full overflow-hidden border border-garuda-black gap-[10px] focus-within:border-[#0068FF] transition-all duration-300">
                        ...
                        <select id="year-select-{{ $loop->index }}" name=""
                            class="date-select year-select appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal cursor-pointer"
                            data-index="{{ $loop->index }}"
                            onchange="updateDateOfBirth({{ $loop->index }})">
                            <option hidden value="">YYYY</option>
                            
                            <!-- LOOP ANGKA TAHUN DI SINI -->
            ...
            <label class="flex flex-col gap-[10px]">
                ...
                <div class="relative flex items-center w-full rounded-full overflow-hidden border border-garuda-black gap-[10px] focus-within:border-[#0068FF] transition-all duration-300">
                    ...
                    <select name="passengers[{{ $loop->index }}][nationality]"
                        class="appearance-none w-full outline-none pl-[50px] py-3 px-5 font-semibold indeterminate:!font-normal cursor-pointer">
                        <option hidden value="">Select country region</option>
...
@section('scripts')
    ...
    <script>
        function updateDateOfBirth(index) {
            ...
            if (day && month && year) {
                document.getElementById(`date_of_birth-${index}`).value = `${year}-${month}-${day}`;
            }
        }
    </script>
@endsection